...
function buildMarkdownReport(verdict) {
    ...
    lines.push('');
    lines.push('> Push a follow-up commit with the fixes; this check re-runs on `synchronize`.');
    return lines.join('\n');
}
...
function buildSlackPayload(verdict, prUrl, prTitle) {
    ...
    if (!verdict.isCompliant) {
        for (const r of verdict.results.filter((x) => !x.isCompliant).slice(0, 10)) {
            const issue = (r.securityVerdict.isCompliant ? '' : r.securityVerdict.issue) +
...
function buildEmailHtml(verdict, prUrl, prTitle) {
    ...
    const rows = verdict.results
        ...
        <tr>
          ...
          <td style="padding:8px;border:1px solid #ddd;">${escapeHtml((r.securityVerdict.isCompliant ? '' : r.securityVerdict.issue) +
        (r.bucketVerdict && !r.bucketVerdict.isCompliant
            ? ` | ${r.bucketVerdict.issue}`
            : ''))}</td>
        </tr>`)
